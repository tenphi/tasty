...
- Local `@keyframes` are owned by the classes whose rules actually name them: one reference however many times they render, released when the last owner is collected. Their injected name is resolved before those rules are written, so a renamed animation reaches the rule that animates it — it did not before, because the correcting write hit a cache key the first write had already claimed. New `holdKeyframes()` and `ownKeyframes()`.
