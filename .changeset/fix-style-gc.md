...
- Local `@keyframes` are owned by the classes whose rules actually name them: one reference however many times they render, released when the last owner is collected. Their injected name is resolved before those rules are written and forms part of the chunk's cache key, so two components authoring the same `animation: fade 1s` over different `@keyframes fade` cannot be handed the same class, and a renamed animation reaches the rule that animates it — it did not before, because the correcting write hit a cache key the first write had already claimed. New `holdKeyframes()` and `ownKeyframes()`.
