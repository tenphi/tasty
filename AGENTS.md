...
| [`docs/runtime.md`](docs/runtime.md) | Runtime API — React-specific `tasty()` factory, component creation, extending, `styleProps`, variants, sub-element styling (`elements` prop, selector affix), and hooks (`useStyles`, `useGlobalStyles`, `useRawCSS`, `useKeyframes`, `useProperty`). |
