...
See the [Usage Guide — Best Practices](usage.md#best-practices) for the full list of do's and don'ts.
...
For full details on merge semantics, `@inherit`, `null`, and `false` tombstones, see the [Usage Guide — Extending vs. Replacing State Maps](usage.md#extending-vs-replacing-state-maps).
