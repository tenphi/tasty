...
function expandExclusiveConditionOrs(
  ...
  // Only expand ORs that involve at-rules (media/container/supports/starting).
  // Pure selector ORs (modifiers, pseudos, root) can be combined with :is()
  // and don't need expansion into exclusive branches.
  if (!orBranches.some(hasAtRuleContext)) {
    return [entry];
  }
